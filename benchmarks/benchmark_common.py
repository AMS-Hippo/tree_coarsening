...
def project_root(root: Path | None = None) -> Path:
    """Return the explicitly supplied repository root or the current directory."""

    root = (root or Path.cwd()).resolve()
    if not (root / "pyproject.toml").is_file() or not (root / "tree_coarsening").is_dir():
        raise RuntimeError(
            "Run benchmarks from the repository root, or pass root=Path('/path/to/tree-coarsening')."
        )
    return root
