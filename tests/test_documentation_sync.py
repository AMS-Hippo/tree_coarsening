...
def test_normative_api_copies_are_byte_identical_and_frozen() -> None:
    normative = (ROOT / "docs" / "api" / "schema1.md").read_bytes()
    assert hashlib.sha256(normative).hexdigest() == FROZEN_SCHEMA_SHA256
    assert (ROOT / "tests" / "baselines" / "FROZEN_SCHEMA.sha256").read_text().split()[0] == (
...
def test_current_documentation_does_not_reintroduce_legacy_api() -> None:
    paths = (
        ROOT / "README.md",
        ROOT / "docs" / "contributing" / "coarseners.md",
        ROOT / "docs" / "design" / "parametric_star_coarsener.md",
        ROOT / "docs" / "design" / "edge_bpe_numba_experiment.md",
        ROOT / "notebooks" / "star_coarsener_example.ipynb",
        ROOT / "notebooks" / "edge_bpe_coarsener_example.ipynb",
        ROOT / "notebooks" / "greedy_bpe_coarsener_example.ipynb",
        ROOT / "notebooks" / "named_vertex_coarsener_example.ipynb",
        ROOT / "notebooks" / "two_stage_star_then_bpe_roundtrip_example.ipynb",
