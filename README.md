...
- `greedy_bpe_coarsener_example.ipynb` — greedy edge-BPE scheduling.
- `named_vertex_coarsener_example.ipynb` — explicit label/UID component coarsening.
- `two_stage_star_then_bpe_roundtrip_example.ipynb` — Star → BPE application and exact reverse decode.

From a fresh checkout, install the package into the notebook kernel first:
...
```python
from tree_coarsening import GreedyBPECoarsener

greedy = GreedyBPECoarsener(
    num_merges=8,             # globally selected seed pairs
    min_pair_count=2,
    pair_score="count",
    backend="python",         # optional "numba"
).fit(training_trees)
...
`GreedyBPECoarsener` reuses the ordinary edge-BPE encoder, decoder, exact-type
contract, and optional Numba backend. The scheduling difference is fit-time
only: after selecting a global seed pair `(A, B)`, it greedily emits ordinary
edge-BPE rules for `(A, B)`, `((A, B), B)`, and so on until that repeated-child
continuation is exhausted before selecting the next seed. This is useful for
compressing repeated sibling runs while preserving schema-1 reversibility.
