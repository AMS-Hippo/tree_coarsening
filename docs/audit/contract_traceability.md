...
This matrix maps the frozen `docs/api/schema1.md` contract to the current
...
| §22 Parametric Star | `coarseners/parametric_star.py` | `test_parametric_star_schema1.py`, raw/encoded differential `test_parametric_star_batching.py` |
| §23 Edge BPE | `coarseners/edge_bpe.py`, protected `edge_bpe_numba.py` | `test_edge_bpe_schema1.py`, `test_edge_bpe_parity_and_protection.py` |
| §24 Named Vertex | `coarseners/named_vertices.py` | `test_named_vertex_schema1.py` |
...
| §31 contributor obligations | shared authoring surface plus method and pipeline suites | `docs/contributing/coarseners.md`, `test_authoring_surface.py`, method/conformance suites |
| §32 locked decisions | frozen contract hash and byte-identical public copies | `test_documentation_sync.py`, `tests/baselines/FROZEN_SCHEMA.sha256` |
...
- The notebooks under `notebooks/` are executable schema-1 syntax
  examples; notebook execution is part of the release audit.
- `docs/audit/SCHEMA_CLARIFICATIONS.md` records implementation interpretations that
